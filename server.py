...
# Optional AI/embedding imports (safe try/except)
try:
...
# local DB models / session (use your existing database.py)
...
# Argon2 is modern and avoids bcrypt 72-byte issues.
...
def hash_password(p: str) -> str:
    ...
    try:
        ...
        # Log the full exception for debugging, then surface a safe error
        print("HASHING EXCEPTION:", repr(e))
        raise
...
recent_messages: List[dict] = []   # {name, text, ts} newest last
recent_embeddings: List[np.ndarray] = []  # aligned suffix for runtime embeddings
current_suggestions: List[str] = []  # persisted latest suggestions
...
def add_to_recent_runtime(name: str, text: str, embedding: Optional[np.ndarray] = None):
    ...
    # trim oldest to respect MAX_RECENT
    if len(recent_messages) > MAX_RECENT:
        del recent_messages[0]
        # keep embeddings aligned as best-effort
        if recent_embeddings and len(recent_embeddings) > len(recent_messages):
...
async def signup(data=Body(...)):
    ...
    try:
        ...
        try:
            ...
            print("hashing error (signup):", repr(e))
...
async def websocket_anonymous(websocket: WebSocket):
    ...
    try:
        ...
        await broadcast_raw(json.dumps({"type":"user_count","auth":total_auth,"anon":total_anon,"total":total_auth+total_anon}))
    except Exception:
        pass
    try:
        while True:
...
async def websocket_auth(websocket: WebSocket, token: str):
    ...
    # send counts + persisted suggestions to this new client
    try:
        total_auth = len(auth_clients)
        total_anon = len(anon_clients)
        await broadcast_raw(json.dumps({"type":"user_count","auth":total_auth,"anon":total_anon,"total":total_auth+total_anon}))
            ...
            try:
                if isinstance(parsed, dict) and parsed.get("type"):
                    ...
                    if t == "message":
                        ...
                        # suggestion pipeline: Groq -> embeddings -> simple fallback
                        suggestions = []
                        ...
                        # add current message to runtime recent buffers (after generating suggestions)
                        if embedding is not None:
                            try:
                        ...
                        # persist & broadcast suggestions (if any)
                        if suggestions:
                            cleaned = []
